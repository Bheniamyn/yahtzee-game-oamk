...
export default Gameboard = ({ route }) => {
  ...
  // Takes place when entering the gameboard for the first time
  useEffect(() => {
  ...
  // Takes place when number of throws changes
  useEffect(() => {
    if (nbrOfThrowsLeft == 0) {
      ...
      totalPoints >= BONUS_POINTS_LIMIT
        ? setStatus("You got the bonus 😄")
        : setStatus("You didn't get the bonus 😔");
    }
  }, [nbrOfThrowsLeft]);

  function selectDice(i) {
    if (nbrOfThrowsLeft == 3) {
      setStatus("No cheating! Throw first before selecting!");
      return;
    }
  ...
  function getDicePointColor(i) {
    return selectedDicePoints[i] ? "#27ae5f50" : "#27ae60";
  }

  function getDiceColor(i) {
    return selectedDices[i] ? "#27ae5f50" : "#27ae60";
  }
